...
public class MainGame extends JPanel implements ActionListener{
    ...
    protected Image wall = new ImageIcon("data/image/5.png").getImage();
    protected Image grass = new ImageIcon("data/image/back_ground_boom.png").getImage();
    protected ArrayList<brick> bricks = new ArrayList<brick>();
    protected ArrayList<dragon> ballooms = new ArrayList<dragon>();
    protected ArrayList<Xac_uop> oneals = new ArrayList<Xac_uop>();
            ...
            while ((temporary = bfr.readLine()) != null){
                ...
                for(int i = 0;i < n; i++){
                    ...
                    if (temporary.charAt(i) == '1'){
                        ballooms.add(new dragon(i*48,k*48));
                        map.data.add(new xOy(i*48,k*48, ' '));
                    }
                    if (temporary.charAt(i) == '2'){
                        oneals.add(new Xac_uop(i*48,k*48));
                        map.data.add(new xOy(i*48,k*48, ' '));
                    }
    ...
    public void checkGameOver(){
        Rectangle bomberBound = new Rectangle(player.bX, player.bY, 48, 48);
        for(dragon bl : ballooms){
            if (!bl.isDead && bomberBound.intersects(new Rectangle(bl.bX, bl.bY, 48, 48))) {
                player.motionStatus = 0;
                player.isDead = true;
            }
        }
        for(Xac_uop ol : oneals){
            if (!ol.isDead && bomberBound.intersects(new Rectangle(ol.bX, ol.bY, 48, 48))) {
                player.motionStatus = 0;
                player.isDead = true;
            }
        }
    ...
    private void drawPlayer(Graphics g){
        ...
        if (!player.isDead){
            ...
            g.drawImage(player.bomberStatus, player.bX, player.bY, 42, 42,this);
        }else{
            ...
            if (player.motionStatus <= MOTION_DELAY/4){
                g.drawImage(player.dead_1, player.bX, player.bY, 48, 48,this);
            }
            else if (player.motionStatus/4 < MOTION_DELAY && player.motionStatus <= 2*MOTION_DELAY/4){
                g.drawImage(player.dead_2, player.bX, player.bY, 48, 48,this);
            }
            else if (2*MOTION_DELAY/4 < player.motionStatus && player.motionStatus <= 3*MOTION_DELAY/4){
                g.drawImage(player.dead_3, player.bX, player.bY, 48, 48,this);
            }
            player.motionStatus++;
        }
    }
    ...
    private void drawBomb(Graphics g){
        ...
        for(int i = 0; i < n; i++){
            ...
            if ((100 < time && time <= 120) || (40 < time && time <= 60)){
                ...
                g.drawImage(player.bombs.get(i).bombStatus, x, y, 48, 48,this);
                ...
                if (time1 == 0){
                    ...
                    g.drawImage(player.bombs.get(i).bomb_exploded_1, x, y, 48 , 48, this);
                    for(player.bombs.get(i).east = 1; player.bombs.get(i).east <= range-1; player.bombs.get(i).east++){
                        ...
                        g.drawImage(player.bombs.get(i).explosion_horizontal_right_last_1, x + player.bombs.get(i).east*48, y, 48, 48, this);
                    }
                    for(player.bombs.get(i).west = 1; player.bombs.get(i).west <= range-1; player.bombs.get(i).west++){
                        ...
                        g.drawImage(player.bombs.get(i).explosion_horizontal_left_last_1, x - player.bombs.get(i).west*48, y, 48, 48, this);
                    }
                    for(player.bombs.get(i).north = 1; player.bombs.get(i).north <= range-1; player.bombs.get(i).north++){
                        ...
                        g.drawImage(player.bombs.get(i).explosion_vertical_top_last_1, x, y - player.bombs.get(i).north*48, 48, 48, this);
                    }
                    for(player.bombs.get(i).south = 1; player.bombs.get(i).south <= range-1; player.bombs.get(i).south++){
                        ...
                        g.drawImage(player.bombs.get(i).explosion_vertical_down_last_1, x, y + player.bombs.get(i).north*48, 48, 48, this);
                    }
        ...
        for(int i = 0; i < n; i++){
            ...
            if (ballooms.get(i).direction == 1 || ballooms.get(i).direction == 3){
                if (motion <= MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).left_1;
                }
                else if (MOTION_DELAY/3 < motion && motion <= 2*MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).left_2;
                }
                else if (motion > 2*MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).left_3;
                }
            }

            if (ballooms.get(i).direction == 2 || ballooms.get(i).direction == 4){
                if (motion <= MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).right_1;
                }
                else if (MOTION_DELAY/3 < motion && motion <= 2*MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).right_2;
                }
                else if (motion > 2*MOTION_DELAY/3){
                    ballooms.get(i).dragonStatus = ballooms.get(i).right_3;
                }
            }
            ...
            g.drawImage(ballooms.get(i).dragonStatus, ballooms.get(i).bX, ballooms.get(i).bY, 48, 48, this);
        }else{
                if (ballooms.get(i).motionStatus == MOTION_DELAY){
                    ...
                    g.drawImage(ballooms.get(i).dead, ballooms.get(i).bX, ballooms.get(i).bY, 48, 48, this);
    ...
    private void drawOneal(Graphics g){
        ...
        for(int i = 0; i < n; i++){
            ...
            //draw(g, oneals.get(i).onealStatus, oneals.get(i).bX + oneals.get(i).onealStatus.offset_X, oneals.get(i).bY + oneals.get(i).onealStatus.offset_Y);
                g.drawImage(oneals.get(i).onealStatus, oneals.get(i).bX, oneals.get(i).bY, 48, 48, this);
        }else{
                if (oneals.get(i).motionStatus == MOTION_DELAY){
                    ...
                    g.drawImage(oneals.get(i).dead, oneals.get(i).bX, oneals.get(i).bY, 48, 48, this);
        ...
        for(int i = 0; i < n; i++){
            ...
            if (!bricks.get(i).isDestroyed){
                ...
                if (time == 0){
                    ...
                    g.drawImage(bricks.get(i).image, x, y, 48, 48, this);
    ...
    public void checkLevelPassed(){
        levelPassed = portal.getBound().intersects(new Rectangle(player.bX, player.bY, 48, 48)) && isClearEnermy();
    }

    //vẽ toàn bộ các vật thể
    private void drawGame(Graphics g){
        for(xOy xoy : map.data){
            if (xoy.type == '#') g.drawImage(wall, xoy.x, xoy.y, 48,48,this);
             else if (xoy.type == ' ' || xoy.type == '*' || xoy.type == 'b') g.drawImage(grass, xoy.x, xoy.y, 48, 48, this);
        }
        g.drawImage(portal.image, portal.x, portal.y, 46, 46,this);
        drawBrick(g);
        drawBomb(g);
        drawBalloom(g);
        drawOneal(g);
